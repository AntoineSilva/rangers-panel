...
export default function LoginPage() {
  ...
  async function handleLogin(e) {
    e.preventDefault()
    snd.carriageReturn()
    if (!email || !password) { setMsg({ text: '⚠ Veuillez remplir tous les champs.', type: 'error' }); return }
    setLoading(true)
    setMsg({ text: '⟳ Vérification en cours...', type: 'success' })
    const { error } = await signIn(email, password)
    setLoading(false)
    if (error) {
      setMsg({ text: '⚠ ' + (error.message === 'Invalid login credentials' ? 'Identifiants incorrects.' : error.message), type: 'error' })
    } else {
      snd.ding()
      setMsg({ text: '✓ Accès autorisé. Chargement...', type: 'success' })
    }
  }
  ...
  async function handleRegister(e) {
    ...
    if (!regEmail) { setMsg({ text: '⚠ Email requis.', type: 'error' }); return }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '620px' }}>
        ...
        {/* Badge header */}
        <div style={{ textAlign: 'center', marginBottom: '16px', animation: 'pageIn .8s ease' }}>
        ...
        <div style={{ background: 'linear-gradient(180deg,#4a3828,#2e1e12,#1e1208)', border: '1px solid #5a4030', borderBottom: '3px solid #150e08', borderRadius: '10px 10px 0 0', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontFamily: "'Special Elite',cursive", fontSize: '8px', letterSpacing: '2px', color: 'rgba(180,150,100,.45)' }}>
            — U.S. RANGERS RECORD MACHINE — PATENT No. 1899 —
          </span>
        </div>
          ...
          <div style={{ display: 'flex', gap: '4px', marginBottom: '-1px', position: 'relative', zIndex: 2 }}>
            {['login', 'register'].map(t => (
              <button key={t} onClick={() => switchTab(t)} style={{
                padding: tab === t ? '8px 18px 10px' : '8px 18px',
                background: tab === t ? 'var(--paper)' : 'var(--paper-aged)',
                border: '1px solid', borderColor: tab === t ? 'var(--ink-2)' : 'var(--ink-3)',
                borderBottom: tab === t ? '1px solid var(--paper)' : '1px solid var(--ink-3)',
                fontFamily: "'Special Elite',cursive", fontSize: '10px', letterSpacing: '2px',
                textTransform: 'uppercase', color: tab === t ? 'var(--ink)' : 'var(--ink-3)',
                cursor: 'pointer', clipPath: 'polygon(0 25%,10% 0,90% 0,100% 25%,100% 100%,0 100%)',
                zIndex: tab === t ? 3 : 1, position: 'relative'
              }}>
                ▸ {t === 'login' ? 'Connexion' : 'Enregistrement'}
              </button>
            ))}
          </div>
            ...
            {tab === 'login' && (
              <form onSubmit={handleLogin}>
                <div className="field-group">
                  <label>Email</label>
                  <input type="email" value={email} onChange={e => { setEmail(e.target.value); snd.keyClick() }} placeholder="ranger@usnew-austin.com" />
                </div>
                <div className="field-group">
                  <label>Mot de passe</label>
                  <input type="password" value={password} onChange={e => { setPassword(e.target.value); snd.keyClick() }} placeholder="••••••••••" />
                </div>
            ...
            {tab === 'register' && (
              <form onSubmit={handleRegister}>
                <div className="field-group">
                  ...
                  <input type="text" value={regCode} onChange={e => { setRegCode(e.target.value.toUpperCase()); snd.keyClick() }} placeholder="USR — XXXX" maxLength={12} style={{ fontSize: '18px', letterSpacing: '8px', textAlign: 'center', fontWeight: 700 }} />
                ...
                <div className="field-group">
                  <label>Email</label>
                  <input type="email" value={regEmail} onChange={e => { setRegEmail(e.target.value); snd.keyClick() }} placeholder="votre@email.com" />
                </div>
                <div className="field-group">
                  <label>Mot de passe</label>
                ...
                <div className="field-group">
                  ...
                  <div onClick={() => document.getElementById('photoInput').click()} style={{ border: '1px dashed var(--ink-3)', padding: '14px', display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', transition: 'all .2s' }}>
                    {photoPreview
