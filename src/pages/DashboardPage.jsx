  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className={`modal-box${wide?' modal-wide':''}`} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px',paddingBottom:'10px',borderBottom:'2px solid var(--ink)'}}>
          <div style={{fontFamily:"'Special Elite',cursive",fontSize:'14px',letterSpacing:'3px',textTransform:'uppercase'}}>{title}</div>
...
function OrgNode({role,name,isTop,onClick}){
  return(
    <div onClick={onClick} style={{border:`${isTop?'2':'1.5'}px solid ${isTop?'var(--red)':'var(--ink-2)'}`,padding:'7px 14px',textAlign:'center',background:'rgba(180,150,80,.07)',minWidth:'150px',cursor:onClick?'pointer':'default',transition:'background .15s'}}
...
function TabAccueil({ranger,switchTab}){
  ...
  useEffect(()=>{
    async function load(){
      ...
      setStats({rangers:r||0,volees:v||0,solde,enquetes:e||0})
    }
    load()
  },[])
  const now=new Date()
  return(
    <div className="page-in">
      <PageHeader title="Tableau de Bord" sub="U.S. Rangers — Bureau de New Austin">
        <div style={{textAlign:'right',fontSize:'9px',letterSpacing:'2px',color:'var(--ink-3)',lineHeight:'1.7'}}>
      ...
      <div style={{marginTop:'18px',display:'flex',gap:'8px',flexWrap:'wrap'}}>
        <button className="btn btn-primary btn-sm" onClick={()=>switchTab('enquetes')}>🔍 Voir les enquêtes</button>
        <button className="btn btn-sm" onClick={()=>switchTab('rapports')}>📄 Rédiger un rapport</button>
...
const COMPETENCES_LIST = [
  'Pistage','Tireur d\'élite','Interrogatoire','Médecine','Droit','Infiltration',
  'Équitation','Cynophile','Explosifs','Navigation','Langues étrangères','Combat rapproché',
  'Photographie','Enquête','Renseignement','Négociation','Surveillance','Camouflage'
]
...
function TabProfil({ranger,snd}){
  ...
  function toggleComp(c){
    snd.keyClick()
    setForm(f=>({...f,competences:f.competences.includes(c)?f.competences.filter(x=>x!==c):[...f.competences,c]}))
  }

  async function save(){
    snd.carriageReturn()
    const{error}=await supabase.from('rangers').update({surnom:form.surnom||null,bio:form.bio||null,competences:form.competences}).eq('id',ranger.id)
    if(error){setMsg('⚠ '+error.message);return}
    snd.ding();setMsg('✓ Profil mis à jour');setEditing(false)
  ...
  return(
    <div className="page-in">
      ...
      {/* Carte identité */}
      <div style={{display:'flex',gap:'24px',alignItems:'flex-start',flexWrap:'wrap',marginBottom:'24px',padding:'16px',border:'1px solid rgba(160,130,70,.35)',background:'rgba(180,150,80,.04)'}}>
        <div style={{width:'90px',height:'112px',border:'2px solid var(--ink)',overflow:'hidden',background:'var(--paper-aged)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'32px',flexShrink:0,filter:'sepia(.3)'}}>
          {ranger.photo_url?<img src={ranger.photo_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:'👤'}
        </div>
        <div style={{flex:1}}>
          <div style={{fontFamily:"'Special Elite',cursive",fontSize:'clamp(18px,4vw,26px)',letterSpacing:'4px',color:'var(--ink)',textTransform:'uppercase'}}>{ranger.prenom_rp} {ranger.nom_rp}</div>
          {ranger.surnom&&<div style={{fontFamily:"'Special Elite',cursive",fontSize:'14px',color:'var(--ink-3)',fontStyle:'italic',marginTop:'2px'}}>« {ranger.surnom} »</div>}
          ...
          <div style={{marginTop:'6px',fontSize:'10px',color:'var(--ink-3)',letterSpacing:'1px'}}>BP : <strong>{ranger.bp}</strong></div>
        </div>
      </div>
      ...
      {editing?(
        <>
          ...
          <Field label="Biographie / Notes personnelles">
            <textarea value={form.bio} onChange={e=>{setForm(f=>({...f,bio:e.target.value}));snd.keyClick()}} placeholder="Décrivez votre parcours, vos spécialités..." style={{minHeight:'100px'}} />
          </Field>
          <div className="field-group">
            <label>Compétences (sélectionnez)</label>
          ...
          <div className="btn-row">
            ...
            <button className="btn" onClick={()=>{setEditing(false);setForm({surnom:ranger.surnom||'',bio:ranger.bio||'',competences:ranger.competences||[]})}}>✕ Annuler</button>
          </div>
        </>
      ):(
        <>
          ...
          {(ranger.competences||[]).length>0?(
            <div style={{display:'flex',flexWrap:'wrap',gap:'4px'}}>
              {(ranger.competences||[]).map(c=><span key={c} className="comp-tag active">{c}</span>)}
            </div>
...
function TabOrganigramme({canEdit,snd}){
  ...
  async function save(){
    ...
    if(error){setMsg('⚠ '+error.message);return}
    snd.ding();setMsg('✓ Modifié');await load();setTimeout(()=>{setModal(false);setMsg('')},700)
  }
  ...
  return(
    <div className="page-in">
      <PageHeader title="Organigramme" sub="Structure hiérarchique">
        {canEdit&&<div style={{fontSize:'9px',color:'var(--ink-3)',fontStyle:'italic'}}>✎ Cliquer sur un membre pour modifier</div>}
      </PageHeader>
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',fontFamily:"'Special Elite',cursive",overflowX:'auto',gap:'0'}}>
        ...
        <div style={{display:'flex',gap:'20px',flexWrap:'wrap',justifyContent:'center'}}>
          {POLES.map(pole=>{
            ...
            return(
              <div key={pole.id} style={{display:'flex',flexDirection:'column',alignItems:'center',minWidth:'145px'}}>
                <VLine/>
        ...
        {byGrade('deputy').length>0&&(
          <><VLine/>
          <div style={{fontSize:'8px',letterSpacing:'2px',textTransform:'uppercase',color:'var(--ink-3)',margin:'3px 0 7px',borderBottom:'1px solid rgba(106,74,26,.3)',paddingBottom:'2px',minWidth:'160px',textAlign:'center'}}>Deputies</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:'5px',justifyContent:'center'}}>
            {byGrade('deputy').map(r=>(
              <div key={r.id} onClick={canEdit?()=>openEdit(r):null} style={{fontSize:'11px',color:'var(--ink-2)',padding:'3px 9px',border:'1px solid rgba(160,130,70,.3)',minWidth:'110px',textAlign:'center',cursor:canEdit?'pointer':'default',fontFamily:"'Special Elite',cursive",transition:'all .15s'}}
                onMouseEnter={e=>canEdit&&(e.currentTarget.style.background='rgba(180,150,80,.12)')}
                onMouseLeave={e=>canEdit&&(e.currentTarget.style.background='')}>
                {r.prenom_rp} {r.nom_rp}
              </div>
      ...
      {modal&&selected&&(
        <Modal title={`Modifier — ${selected.prenom_rp} ${selected.nom_rp}`} onClose={()=>setModal(false)}>
          ...
          <div className="two-col">
            ...
            <Field label="Rôle spécial"><input type="text" value={form.role_special} onChange={e=>{setForm(f=>({...f,role_special:e.target.value}));snd.keyClick()}} placeholder="Œil de lynx..."/></Field>
          </div>
          <div className="btn-row">
            <button className="btn btn-primary" onClick={save}>✓ Enregistrer</button>
    ...
    <div className="page-in">
            ...
            {items.map(item=>(
              <tr key={item.id}>
                ...
                <td><div style={{display:'flex',gap:'2px'}}>
                  <button className="btn btn-success btn-sm" onClick={()=>openMouv(item)} title="Mouvement">±</button>
      ...
      {(modal==='add'||modal==='edit')&&(
        <Modal title={editItem?`Modifier — ${editItem.article}`:'Nouvel article'} onClose={()=>setModal(false)}>
          ...
          <Field label="Stock min. Blackwater (alerte si en-dessous)"><input type="number" min="0" value={form.min_blackwater} onChange={e=>{setForm(f=>({...f,min_blackwater:+e.target.value}));snd.keyClick()}}/></Field>
      ...
      {modal==='mouv'&&editItem&&(
        <Modal title={`Mouvement — ${editItem.article}`} onClose={()=>setModal(false)}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'14px'}}>
            {POSTES.map(p=>(
              <div key={p.id} style={{border:`2px solid ${mouv.poste===p.id?'var(--ink)':'rgba(160,130,70,.3)'}`,padding:'8px',textAlign:'center',cursor:'pointer',background:mouv.poste===p.id?'rgba(26,16,8,.06)':'transparent',transition:'all .15s'}} onClick={()=>{setMouv(m=>({...m,poste:p.id}));snd.keyClick()}}>
                ...
                <div style={{fontSize:'18px',fontWeight:700,color:'var(--ink)',marginTop:'3px'}}>{editItem[`qte_${p.id}`]||0}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{marginTop:'10px',padding:'9px',background:'rgba(180,150,80,.08)',border:'1px solid rgba(160,130,70,.3)',fontSize:'11px',color:'var(--ink-2)'}}>
          ...
          <div className="btn-row"><button className="btn btn-primary" onClick={saveMouv}>✓ Confirmer</button><button className="btn" onClick={()=>setModal(false)}>✕ Annuler</button></div>
        </Modal>
      )}
      {modal==='histo'&&(
    ...
    <div className="page-in">
      <PageHeader title="Stock d'Armes" sub="Registre des armes à feu">
        <div className="btn-row" style={{margin:0,gap:'5px'}}>
          ...
          <button className="btn btn-danger btn-sm" onClick={()=>{snd.keyClick();setEditArme(null);setForm(f=>({...f,statut:'volee',type_arme:'',numero_serie:''}));setMsg('');setModal(true)}}>⚠ Vol</button>
        </div>
      </PageHeader>
          ...
          <tbody>
            {armes.map(a=>(
              <tr key={a.id} style={a.statut==='volee'?{background:'rgba(139,26,26,.06)'}:{}}>
                ...
                <td style={{fontSize:'11px'}}>{a.ranger?`${a.ranger.prenom_rp} ${a.ranger.nom_rp}`:(a.emplacement||'—')}</td>
      ...
      {modal&&(
        <Modal title={editArme?`Modifier — ${editArme.type_arme}`:'Nouvelle arme'} onClose={()=>setModal(false)}>
          ...
          {form.statut==='affectee'
            ?<Field label="Affectée à"><select value={form.affecte_a} onChange={e=>{setForm(f=>({...f,affecte_a:e.target.value}));snd.keyClick()}}><option value="">— Ranger —</option>{rangers.map(r=><option key={r.id} value={r.id}>{r.prenom_rp} {r.nom_rp}</option>)}</select></Field>
...
function TabComptes({snd,ranger}){
  ...
  const [form,setForm]=useState({objet:'',nom_prenom:'',type_op:'Règlement de facture',autre_precis:'',montant:''})
  const [msg,setMsg]=useState('')
  useEffect(()=>{load()},[])
  async function load(){
  ...
  function openModal(t){snd.keyClick();setType(t);setForm({objet:'',nom_prenom:'',type_op:'Règlement de facture',autre_precis:'',montant:''});setMsg('');setModal(true)}
  async function save(){
    snd.carriageReturn()
    if(!form.montant){setMsg('⚠ Montant requis');return}
    const typePermis=form.type_op==='Autre'?(form.autre_precis||'Autre'):form.type_op
    const objet=form.objet||(form.type_op==='Autre'?form.autre_precis:form.type_op)
  ...
  return(
    <div className="page-in">
      ...
      {modal&&(
        <Modal title={type==='ajout'?'Nouvelle entrée — Ajout':'Nouvelle sortie — Retrait'} onClose={()=>setModal(false)}>
          {msg&&<div className={msg.startsWith('⚠')?'msg-error':'msg-success'}>{msg}</div>}
          <Field label="Type d'opération">
          ...
          {form.type_op==='Autre'&&(
            <Field label="Préciser l'opération ★">
              <input type="text" value={form.autre_precis} onChange={e=>{setForm(f=>({...f,autre_precis:e.target.value,objet:e.target.value}));snd.keyClick()}} placeholder="Décrivez l'opération..."/>
            </Field>
          )}
          {form.type_op!=='Autre'&&(
            <Field label="Description complémentaire (optionnel)">
              <input type="text" value={form.objet} onChange={e=>{setForm(f=>({...f,objet:e.target.value}));snd.keyClick()}} placeholder="ex: Facture Valentine, Don Marshall..."/>
            </Field>
          )}
          ...
          <Field label={`Montant ($) — ${type==='ajout'?'Entrée ↑':'Sortie ↓'}`}>
            <input type="number" step="0.01" min="0" value={form.montant} onChange={e=>{setForm(f=>({...f,montant:e.target.value}));snd.keyClick()}} placeholder="0.00" style={{fontSize:'22px',fontWeight:700}}/>
          </Field>
          <div className="btn-row"><button className="btn btn-primary" onClick={save}>✓ Enregistrer</button><button className="btn" onClick={()=>setModal(false)}>✕ Annuler</button></div>
        </Modal>
      )}
...
function TabRapports({ranger,snd}){
  ...
  async function submit(){
    ...
    if(error){setMsg('⚠ '+error.message);return}
    snd.ding();setMsg(`✓ Télégramme n° ${tel}`);await load();setTimeout(()=>{setModal(false);setMsg('')},900)
  }
  return(
    <div className="page-in">
      ...
      {modal&&(
        <Modal title="Nouveau Rapport" onClose={()=>setModal(false)} wide>
          ...
          <Field label="Contenu ★"><textarea value={form.contenu} onChange={e=>{setForm(f=>({...f,contenu:e.target.value}));snd.keyClick()}} placeholder="Je soussigné(e)..." style={{minHeight:'130px'}}/></Field>
          <Field label="Éléments supplémentaires"><textarea style={{minHeight:'60px'}} value={form.elements_supp} onChange={e=>{setForm(f=>({...f,elements_supp:e.target.value}));snd.keyClick()}} placeholder="Pièces à conviction..."/></Field>
      ...
      {viewModal&&(
        <Modal title={`Rapport n° ${viewModal.numero_telegram}`} onClose={()=>setViewModal(null)} wide>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'12px',fontSize:'10px',color:'var(--ink-3)'}}>
          ...
          <div style={{borderTop:'1px solid var(--ink-3)',paddingTop:'10px',whiteSpace:'pre-wrap',fontStyle:'italic',fontSize:'13px',lineHeight:'27px',color:'var(--ink-2)'}}>{viewModal.contenu}</div>
...
function TabEnquetes({ranger,snd}){
  ...
  const [modal,setModal]=useState(false) // 'new-enquete' | 'new-elem' | 'view-elem'
  const [rangers,setRangers]=useState([])
  ...
  const [viewElem,setViewElem]=useState(null)
  const [msg,setMsg]=useState('')
  ...
  function openDossier(e){snd.keyClick();setSelected(e);loadElements(e.id)}
  function closeDossier(){snd.keyClick();setSelected(null);setElements([])}

  async function createEnquete(){
    snd.carriageReturn()
    if(!form.titre){setMsg('⚠ Titre requis');return}
    const{data,error}=await supabase.from('enquetes').insert({
      titre:form.titre,description:form.description,type_enquete:form.type_enquete,
      priorite:form.priorite,lieu_principal:form.lieu_principal||null,
      comtes:form.comtes?form.comtes.split('·').map(s=>s.trim()).filter(Boolean):[],
      date_debut:form.date_debut,statut:'en_cours',cree_par:ranger?.id,
      rangers_assignes:[ranger?.id].filter(Boolean),
    }).select().single()
    if(error){setMsg('⚠ '+error.message);return}
    snd.ding();await load();setModal(false);setSelected(data);loadElements(data.id)
  }
  ...
  async function updateStatut(enq,statut){
    ...
    await load()
    if(selected?.id===enq.id) setSelected(s=>({...s,statut}))
  }

  async function addElement(){
    snd.carriageReturn()
    if(!elemForm.titre){setMsg('⚠ Titre requis');return}
    const{error}=await supabase.from('enquete_elements').insert({
      enquete_id:selected.id,type_elem:elemForm.type_elem,titre:elemForm.titre,
      contenu:elemForm.contenu||null,lieu:elemForm.lieu||null,
      date_evenement:elemForm.date_evenement||null,importance:elemForm.importance,
      photo_url:elemForm.photo_url||null,ajoute_par:ranger?.id,
    })
    if(error){setMsg('⚠ '+error.message);return}
    snd.ding();await loadElements(selected.id);setModal(false)
  ...
  async function deleteElement(id){
    ...
    snd.carriageReturn()
    await supabase.from('enquete_elements').delete().eq('id',id)
    await loadElements(selected.id)
  }

  async function deleteEnquete(id){
    if(!confirm('Supprimer ce dossier d\'enquête ?'))return
    snd.carriageReturn()
    await supabase.from('enquetes').delete().eq('id',id)
    closeDossier();await load()
  }
  ...
  const TYPES_ELEM=[
    {id:'note',label:'Note d\'enquête',icon:'📝'},
    {id:'preuve',label:'Preuve / Indice',icon:'🔬'},
    {id:'temoin',label:'Témoignage',icon:'👁'},
    {id:'photo',label:'Photo / Description',icon:'📷'},
    {id:'lieu',label:'Lieu d\'intérêt',icon:'📍'},
    {id:'suspect',label:'Suspect / Cible',icon:'🎯'},
    {id:'rapport_lié',label:'Rapport lié',icon:'📄'},
  ]

  // ── Vue dossier ──
  if(selected){
    const statutColor={en_cours:'#2a5a7a',en_traque:'#8a6020',resolue:' var(--green)',classee:'var(--ink-3)'}
    return(
      <div className="page-in">
        <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px',paddingBottom:'10px',borderBottom:'2px solid var(--ink)',flexWrap:'wrap'}}>
          <button className="btn btn-sm" onClick={closeDossier}>← Retour</button>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:'9px',letterSpacing:'2px',color:'var(--ink-3)',marginTop:'2px'}}>Dossier #{selected.id.slice(0,8).toUpperCase()} · {selected.type_enquete}</div>
          </div>
          <div style={{display:'flex',gap:'5px',flexWrap:'wrap'}}>
            <select value={selected.statut} onChange={e=>updateStatut(selected,e.target.value)} style={{fontFamily:"'Special Elite',cursive",fontSize:'10px',letterSpacing:'2px',padding:'4px 8px',border:'1.5px solid var(--ink-2)',background:'var(--paper)',color:'var(--ink)',cursor:'pointer',textTransform:'uppercase'}}>
              <option value="en_cours">🔍 En cours</option>
              <option value="en_traque">🎯 En traque</option>
              <option value="resolue">✓ Résolue</option>
              <option value="classee">📁 Classée</option>
            </select>
            <button className="btn btn-primary btn-sm" onClick={()=>{snd.keyClick();setMsg('');setModal('new-elem')}}>+ Ajouter</button>
        ...
        {/* Infos dossier */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'10px',marginBottom:'18px',padding:'12px',border:'1px solid rgba(160,130,70,.3)',background:'rgba(180,150,80,.04)'}}>
          <div><div style={{fontSize:'8px',letterSpacing:'2px',color:'var(--ink-3)',textTransform:'uppercase',marginBottom:'2px'}}>Statut</div><div style={{fontFamily:"'Special Elite',cursive",fontSize:'12px',letterSpacing:'2px',color:statutColor[selected.statut]||'var(--ink)'}}>{STATUT_ENQUETE[selected.statut]}</div></div>
          <div><div style={{fontSize:'8px',letterSpacing:'2px',color:'var(--ink-3)',textTransform:'uppercase',marginBottom:'2px'}}>Priorité</div><div style={{fontFamily:"'Special Elite',cursive",fontSize:'12px',letterSpacing:'2px',color:PRIORITE_COLORS[selected.priorite]||'var(--ink)',textTransform:'uppercase'}}>{selected.priorite}</div></div>
          <div><div style={{fontSize:'8px',letterSpacing:'2px',color:'var(--ink-3)',textTransform:'uppercase',marginBottom:'2px'}}>Lieu principal</div><div style={{fontSize:'12px',color:'var(--ink-2)'}}>{selected.lieu_principal||'—'}</div></div>
          <div><div style={{fontSize:'8px',letterSpacing:'2px',color:'var(--ink-3)',textTransform:'uppercase',marginBottom:'2px'}}>Ouvert le</div><div style={{fontSize:'12px',color:'var(--ink-2)'}}>{selected.date_debut?new Date(selected.date_debut).toLocaleDateString('fr-FR'):'—'}</div></div>
          {(selected.comtes||[]).length>0&&<div><div style={{fontSize:'8px',letterSpacing:'2px',color:'var(--ink-3)',textTransform:'uppercase',marginBottom:'2px'}}>Comtés</div><div style={{fontSize:'11px',color:'var(--ink-2)'}}>{(selected.comtes||[]).join(', ')}</div></div>}
        </div>

        {selected.description&&(
          <div style={{marginBottom:'16px',padding:'10px 14px',borderLeft:'3px solid var(--ink-3)',background:'rgba(180,150,80,.05)',fontSize:'12px',lineHeight:'26px',color:'var(--ink-2)',fontStyle:'italic'}}>
            {selected.description}
          </div>
        )}

        {/* Suspects */}
        {(elements.filter(e=>e.type_elem==='suspect').length>0)&&(
          <div style={{marginBottom:'16px'}}>
            <div className="section-title">🎯 Suspects / Cibles ({elements.filter(e=>e.type_elem==='suspect').length})</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:'8px'}}>
              {elements.filter(e=>e.type_elem==='suspect').map(e=>(
                <div key={e.id} style={{border:`1px solid ${e.importance==='critique'?'var(--red)':'rgba(160,130,70,.35)'}`,padding:'10px 12px',background:e.importance==='critique'?'rgba(139,26,26,.05)':'rgba(180,150,80,.04)'}}>
                  ...
                  {e.contenu&&<div style={{fontSize:'11px',color:'var(--ink-2)',marginTop:'5px',fontStyle:'italic'}}>{e.contenu}</div>}
                  {e.photo_url&&<img src={e.photo_url} alt="" style={{width:'100%',marginTop:'6px',filter:'sepia(.3)',border:'1px solid var(--ink-3)'}} onError={ev=>ev.target.style.display='none'}/>}
        ...
        {/* Éléments */}
        <div className="section-title">📂 Éléments du dossier ({elements.filter(e=>e.type_elem!=='suspect').length})</div>
        {elements.filter(e=>e.type_elem!=='suspect').length===0&&(
          <div style={{color:'var(--ink-3)',fontStyle:'italic',fontSize:'12px',marginBottom:'14px',padding:'14px',border:'1px dashed rgba(160,130,70,.3)',textAlign:'center'}}>
            Aucun élément — Cliquez sur « + Ajouter » pour enrichir le dossier
          </div>
        )}
        {elements.filter(e=>e.type_elem!=='suspect').map(e=>(
          <div key={e.id} className={`elem-card ${e.importance==='critique'?'critique':''}`} data-icon={ELEM_ICONS[e.type_elem]||'📝'}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
              <div style={{flex:1}}>
                ...
                {e.contenu&&<div style={{fontSize:'12px',color:'var(--ink-2)',marginTop:'4px',fontStyle:'italic',lineHeight:'22px'}}>{e.contenu}</div>}
                {e.photo_url&&<div style={{marginTop:'8px'}}><img src={e.photo_url} alt="" style={{maxWidth:'200px',maxHeight:'150px',objectFit:'cover',filter:'sepia(.3)',border:'1px solid var(--ink-3)'}} onError={ev=>ev.target.style.display='none'}/></div>}
                <div style={{fontSize:'9px',color:'var(--ink-3)',marginTop:'4px'}}>
                  Ajouté par {e.ranger?`${e.ranger.prenom_rp} ${e.ranger.nom_rp}`:'—'} · {new Date(e.created_at).toLocaleDateString('fr-FR')}
                </div>
              </div>
        ...
        {modal==='new-elem'&&(
          <Modal title="Ajouter un élément" onClose={()=>setModal(false)} wide>
            {msg&&<div className={msg.startsWith('⚠')?'msg-error':'msg-success'}>{msg}</div>}
            <Field label="Type d'élément">
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:'5px',marginTop:'4px'}}>
                {TYPES_ELEM.map(t=>(
                  <div key={t.id} onClick={()=>{setElemForm(f=>({...f,type_elem:t.id}));snd.keyClick()}} style={{border:`1.5px solid ${elemForm.type_elem===t.id?'var(--ink)':'rgba(160,130,70,.35)'}`,padding:'6px 8px',cursor:'pointer',textAlign:'center',background:elemForm.type_elem===t.id?'rgba(26,16,8,.06)':'transparent',transition:'all .15s',fontFamily:"'Special Elite',cursive",fontSize:'9px',letterSpacing:'1px'}}>
                    {t.icon} {t.label}
                  </div>
                ))}
              </div>
            ...
            <div className="two-col">
              ...
              <Field label="Importance"><select value={elemForm.importance} onChange={e=>{setElemForm(f=>({...f,importance:e.target.value}));snd.keyClick()}}>{IMPORTANCES.map(i=><option key={i}>{i}</option>)}</select></Field>
            </div>
            <div className="two-col">
              <Field label="Lieu (si applicable)"><input type="text" value={elemForm.lieu} onChange={e=>{setElemForm(f=>({...f,lieu:e.target.value}));snd.keyClick()}} placeholder="ex: Fort Mercer, Armadillo..."/></Field>
            ...
            <Field label="Description / Contenu"><textarea value={elemForm.contenu} onChange={e=>{setElemForm(f=>({...f,contenu:e.target.value}));snd.keyClick()}} placeholder="Description détaillée, témoignage, notes..." style={{minHeight:'90px'}}/></Field>
            <Field label="URL Photo / Image (optionnel)"><input type="text" value={elemForm.photo_url} onChange={e=>{setElemForm(f=>({...f,photo_url:e.target.value}));snd.keyClick()}} placeholder="https://..."/></Field>
            {elemForm.photo_url&&<div style={{marginBottom:'10px'}}><img src={elemForm.photo_url} alt="" style={{maxWidth:'120px',maxHeight:'90px',objectFit:'cover',filter:'sepia(.3)',border:'1px solid var(--ink-3)'}} onError={ev=>ev.target.style.display='none'}/></div>}
            <div className="btn-row"><button className="btn btn-primary" onClick={addElement}>✓ Ajouter au dossier</button><button className="btn" onClick={()=>setModal(false)}>✕ Annuler</button></div>
          </Modal>
        )}
      </div>
    )
  ...
  return(
    <div className="page-in">
      <PageHeader title="Enquêtes" sub="Bureau des investigations — U.S. Rangers">
        <button className="btn btn-primary btn-sm" onClick={()=>{snd.keyClick();setForm({titre:'',description:'',type_enquete:'Générale',priorite:'normale',lieu_principal:'',comtes:'',date_debut:new Date().toISOString().split('T')[0]});setMsg('');setModal('new-enquete')}}>+ Ouvrir un dossier</button>
      </PageHeader>

      {/* Filtres */}
      <div style={{display:'flex',gap:'5px',flexWrap:'wrap',marginBottom:'16px'}}>
        {['all','en_cours','en_traque','resolue','classee'].map(s=>(
          <button key={s} className={`btn btn-sm ${filterStatut===s?'btn-primary':''}`} onClick={()=>{snd.keyClick();setFilterStatut(s)}}>
            {s==='all'?'Toutes':STATUT_ENQUETE[s]}
          </button>
        ))}
      </div>

      {/* Stats */}
      <div className="info-grid" style={{gridTemplateColumns:'repeat(4,1fr)',marginBottom:'16px'}}>
      ...
      {loading?<Loader/>:(
        filtered.length===0
          ?<div style={{textAlign:'center',padding:'32px',fontStyle:'italic',color:'var(--ink-3)',border:'1px dashed rgba(160,130,70,.3)'}}>Aucun dossier d'enquête — Cliquez sur « + Ouvrir un dossier »</div>
          :filtered.map(e=>(
            <div key={e.id} className={`enquete-card ${e.priorite}`} onClick={()=>openDossier(e)}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'10px',flexWrap:'wrap'}}>
                <div style={{flex:1}}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap',marginBottom:'4px'}}>
                    <span style={{fontFamily:"'Special Elite',cursive",fontSize:'14px',letterSpacing:'2px',color:'var(--ink)'}}>{e.titre}</span>
                    <span style={{fontFamily:"'Special Elite',cursive",fontSize:'9px',letterSpacing:'2px',color:PRIORITE_COLORS[e.priorite]||'var(--ink-3)',border:`1px solid ${PRIORITE_COLORS[e.priorite]||'var(--ink-3)'}`,padding:'1px 6px',textTransform:'uppercase'}}>{e.priorite}</span>
                  </div>
                  <div style={{fontSize:'10px',color:'var(--ink-3)',display:'flex',gap:'12px',flexWrap:'wrap'}}>
                  ...
                  {e.description&&<div style={{fontSize:'11px',color:'var(--ink-2)',marginTop:'5px',fontStyle:'italic'}}>{e.description.slice(0,120)}{e.description.length>120?'...':''}</div>}
                </div>
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px',flexShrink:0}}>
                  ...
                  <span style={{fontSize:'9px',color:'var(--ink-3)'}}>▶ Ouvrir le dossier</span>
                </div>
              </div>
            </div>
          ))
      ...
      {modal==='new-enquete'&&(
        <Modal title="Ouvrir un dossier d'enquête" onClose={()=>setModal(false)} wide>
          {msg&&<div className={msg.startsWith('⚠')?'msg-error':'msg-success'}>{msg}</div>}
          ...
          <div className="two-col">
            <Field label="Lieu principal"><input type="text" value={form.lieu_principal} onChange={e=>{setForm(f=>({...f,lieu_principal:e.target.value}));snd.keyClick()}} placeholder="ex: Fort Mercer, New Austin..."/></Field>
            <Field label="Comtés concernés (séparés par ·)"><input type="text" value={form.comtes} onChange={e=>{setForm(f=>({...f,comtes:e.target.value}));snd.keyClick()}} placeholder="New Austin · Cholla Springs"/></Field>
          </div>
          ...
          <div className="btn-row"><button className="btn btn-primary" onClick={createEnquete}>▶ Ouvrir le dossier</button><button className="btn" onClick={()=>setModal(false)}>✕ Annuler</button></div>
        </Modal>
      )}
    </div>
  )
...
function TabAdmin({isAdmin,currentRanger,snd}){
  ...
  function openEdit(r){if(!isAdmin)return;snd.keyClick();setEditRanger(r);setForm({grade:r.grade,pole:r.pole||'',is_admin:r.is_admin||false})}
  ...
  return(
    <div className="page-in">
      <PageHeader title="Administration" sub="Gestion des accès"/>
        ...
        <tbody>
          {actifs.map(r=>(
            <tr key={r.id}>
              ...
              <td>{r.is_admin?<span style={{color:'var(--red)',fontFamily:"'Special Elite',cursive",fontSize:'10px',letterSpacing:'1px'}}>★ Admin</span>:<span style={{color:'var(--ink-3)',fontSize:'10px'}}>—</span>}</td>
      ...
      {editRanger&&(
        <Modal title={`Modifier — ${editRanger.prenom_rp} ${editRanger.nom_rp}`} onClose={()=>setEditRanger(null)}>
          ...
          <div className="field-group">
            ...
            <label style={{display:'flex',alignItems:'center',gap:'9px',cursor:'pointer',marginTop:'7px',fontFamily:"'Special Elite',cursive",fontSize:'11px',letterSpacing:'2px'}}>
              <input type="checkbox" checked={form.is_admin} onChange={e=>{setForm(f=>({...f,is_admin:e.target.checked}));snd.keyClick()}} style={{accentColor:'var(--ink)',width:'15px',height:'15px'}}/>
